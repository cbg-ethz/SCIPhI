...
int readParameters(Config<TTreeType> & config, int argc, char* argv[]){
	...
	parseConfig.add_options()
		...
		("bns", boost::program_options::value<decltype(config.maxSupInControlBulk)>(&config.minSupport), "Loci with up to this number of alternative supporting reads in the bulk control sample will be skiped. [2]")
		("bnc", boost::program_options::value<decltype(config.minCovInControlBulk)>(&config.minCoverage), "Minimum required coverage of reads in the bulk control sample. [6]")
