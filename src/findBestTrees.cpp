...
int readParameters(Config<TTreeType> & config, int argc, char* argv[]){
	...
	parseConfig.add_options()
		...
		(",r", boost::program_options::value<decltype(config.reps)>(&config.reps), "Number of repetitions.")
		(",l", boost::program_options::value<decltype(config.loops)>(&config.loops), "Maximal number of iterations per repetition.")
		("lz", boost::program_options::value<decltype(config.learnZygocity)>(&config.learnZygocity), "Set to 1 if zygocity should be learned.")
		("ls", boost::program_options::value<decltype(config.sampleLoops)>(&config.sampleLoops), "Number of sample iterations.")
		("pr", boost::program_options::value<decltype(config.priorMutationRate)>(&config.priorMutationRate), "Prior mutation rate.")
		...
		(",e", boost::program_options::value<decltype(config.paramsEstimateRate)>(&config.paramsEstimateRate), "Paramter estimation rate, i.e. the frection of loops used to estimate the different parameters.")
		("seed", boost::program_options::value<decltype(config.fixedSeed)>(&config.fixedSeed), "Seed for the random number generator.")
		(",t", boost::program_options::value<decltype(config.scoreType)>(&config.scoreType), "Tree score type [m (max), s (sum)].")
		("wildOverDis", boost::program_options::value<double>(&std::get<0>(config.params[0])), "Overdispersion for wild type.")
		("mutationOverDis", boost::program_options::value<double>(&std::get<0>(config.params[1])), "Overdispersion for mutant type.")
		("wildMean", boost::program_options::value<double>(&std::get<0>(config.params[2])), "Mean error rate.")
		("cwm", boost::program_options::value<decltype(config.numCellWithMutationMin)>(&config.numCellWithMutationMin), "Number of cells requiered to have a mutation in order to be called.")
		("mc", boost::program_options::value<decltype(config.minCoverage)>(&config.minCoverage), "Minimum coverage required per cell.")
		("mcc", boost::program_options::value<decltype(config.minCoverageAcrossCells)>(&config.minCoverageAcrossCells), "Minimum coverage required by at least x cells which support the alternative.")
		...
		("ese", boost::program_options::value<decltype(config.estimateSeqErrorRate)>(&config.estimateSeqErrorRate), "Estimate the sequencing error rate");
