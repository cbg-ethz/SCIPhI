...
int readParameters(Config<TTreeType> & config, int argc, char* argv[]){
	...
	parseConfig.add_options()
		...
		("cwm", boost::program_options::value<decltype(config.numCellWithMutationMin)>(&config.numCellWithMutationMin), "Number of cells requiered to have a mutation in order to be called. [2]")
